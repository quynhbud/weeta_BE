...
const loginUserWithEmailAndPassword = async (email, password) => {
    ...
    if (!(await accountService.checkVerifyEmail(user._id)))
        throw new AppError(httpStatus.UNAUTHORIZED, 'Chưa xác nhận email');
    if (!(await accountService.checkIsActive(user._id)))
        throw new AppError(httpStatus.UNAUTHORIZED, 'Tài khoản đang bị chặn');
    return user;
};
