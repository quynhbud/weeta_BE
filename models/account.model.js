...
AccountSchema.statics.usernameExists = async function (username, excludeUserId) {
  const account = await this.findOne({ username, _id: { $ne: excludeUserId } });
  return !!account;
};

AccountSchema.statics.isPhoneTaken = async function (phoneNumber, excludeUserId) {
  const account = await this.findOne({ phoneNumber, _id: { $ne: excludeUserId } });
  return !!account;
};
/**
